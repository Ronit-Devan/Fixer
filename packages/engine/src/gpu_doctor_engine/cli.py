...
def _load_trace_or_exit(trace_path: Path):
    """Load a trace, turning malformed input into a clean error (exit 1).

    Mirrors the exception set the web API catches around the same call: invalid
    JSON / gzip, or valid JSON that is not a profiler trace object. A traceback
    here is a user-input problem, not an engine failure.
    """
    try:
        return load_trace(trace_path)
    except (ValueError, OSError, AttributeError, TypeError, KeyError) as e:
        err_console.print(
            f"[red]Error:[/red] {trace_path} is not a readable PyTorch Profiler "
...
def report(
    ...
    if output is None:
        ...
        try:
            ...
            err_console.print(f"[red]Error:[/red] cannot write {output}: {e}")
            raise typer.Exit(1)
...
---
_Generated by ET v{__version__}. Research foundation: MinatoLoader, eGPU, DataStates-LLM. github.com/Ronit-Devan/Fixer_
"""
